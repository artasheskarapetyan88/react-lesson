...
function Post({ count, message, id }) {
  ...
  let x = 0;
  const { removePost, plusLikeCount, minusLikeCount,changePost} = usePostsActions();
  ...
  const removeCurrentPost = () => {
    ...
    console.log(newPostList, 9999999);
    removePost(newPostList);
  };
  const openPostChangeArea = (id,message) =>{
    setOpenId(id)
    setText(message)
  }
  ...
  return (
    <div className={c.item}>
      ...
      {message}
        ...
        {!openId ?<span>
          <button onClick={() =>openPostChangeArea(id,message)}>EDIT POST</button>
        </span>:
        <div>
          <span>
            <input defaultValue={message} onChange={handelChangePost}/>
            {!text ? <p>Post field is recuared</p>: null}
          </span>
          <span>
            <button onClick={changeCurentPost}>CHANGE POST</button>
          </span>
        </div>}
      </div>
      ...
      <div>
        <span>Like {like}</span>
        {count < 0 ? <span>bad post</span> : null}
        <button onClick={likeCountUp}> Like </button>
        <button onClick={likeCountDown}> Dislike </button>
      </div>
      <button onClick={removeCurrentPost}>Remove</button>
    </div>
  );
}
export default Post;
